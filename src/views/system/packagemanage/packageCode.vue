...
<script>
import { newDept, getCommodityQrCode } from '@/api/modular/system/posManage'
export default {
  data() {
    return {
      extraImage: '',
  ...
  methods: {
    ...
    add(record) {
      this.record = {}
      this.record = record
      this.title=record.packageName+' - 二维码'
      this.visible = true
      getCommodityQrCode({ commodityId: record.commodityId }).then((res) => {
        if (res.code == 0) {
          // this.data = res.data
