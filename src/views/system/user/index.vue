            ...
            <a-row :gutter="48">
              <a-col :md="8" :sm="24">
                <a-form-item label="关键词" >
                  <a-input v-model="queryParam.searchValue" allow-clear  placeholder="请输入姓名、账号、手机号" @keyup.enter="$refs.table.refresh(true)"/>
                </a-form-item>
              </a-col>
              <a-col :md="8" :sm="24">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sexFilter (sex) {
        const sex_value = this.sexDictTypeDropDown.filter(item => item.code == sex)
        if(sex_value.length>0){
          return sex_value[0].value
        }
      },
      statusFilter (status) {
