...
<script>
import { add, info } from '@/api/modular/system/sysapp'
export default {
  data() {
    return {
      labelCol: {
        ...
        if (res.code === 0){
          setTimeout(() => {
            this.form.setFieldsValue({
              ...
              visibleStatus: res.data.visibleStatus + '',
              sortNo: res.data.sortNo,
              logo: res.data.logo,
              indexUrl: res.data.indexUrl,
              remark: res.data.remark
            })
          })
          this.onStatusChange(res.data.status===1 ? true : false)
        }else {
