      ...
      <div class="div-line-wrap">
        <div class="div-left">
          ...
          <a-select v-model="checkData.categoryId" allow-clear placeholder="请选择科室">
            <a-select-option v-for="(item, index) in ksTypeData" :key="index" :value="item.code">{{
              item.value
            }}</a-select-option>
          </a-select>
        </div>
...
import { saveArticle } from '@/api/modular/system/posManage'
...
export default {
  ...
  methods: {
    ...
    this.ksTypeData.forEach(item=>{
      if(this.checkData.categoryId==item.code)
        this.checkData.categoryName=item.value
    })
