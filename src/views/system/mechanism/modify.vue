<template>
  ...
  import { save, queryHospitalLevel, queryHospitalType,parent,queryHospitaldetail } from '@/api/modular/system/posManage'
  ...
  export default {
    ...
    methods: {
      ...
      modify(record) {
        ...
        console.log("机构ID：",record.hospitalId)
      ...
      getHospitalDetailOut(){
        queryHospitaldetail({hospitalId:this.hospitalId}).then((res)=>{
             if(res.code==0){
                ...
                this.queryParams.hospitalType= res.data.hospitalType.description 
                this.queryParams.level= res.data.level.value 
