          ...
          <div class="draw-bottom">
            ...
            <div class="bottom-down">
              ...
              >
                <div class="out-top">
                  <a-icon
                    :type="itemOut.outIcon"
                    @click="onHideAndSee(itemOut, indexOut)"
                  />
                  ...
                  <span
                    class="out-top-title"
                    @click="onChangeOut(itemOut, indexOut)"
                    style="margin-left: 1%"
                    >{{ itemOut.subjectClassifyName }}</span
                  >
                </div>
                ...
                >
                  ...
                  <span
                    class="out-list-title"
                    @click="onChangeIn(itemChild, indexChild, itemOut, indexOut)"
                    >{{ itemChild.subjectClassifyName }}</span
                  >
                </div>
              </div>
            </div>
          ...
          >
            <span slot="action" slot-scope="text, record">
              ...
              <a-popconfirm
                ...
                ok-text="确定"
                cancel-text="取消"
                @confirm="deleteDiseaseOut(record)"
              >
...
export default {
  ...
  methods: {
    ...
    onChangeIn(itemChild, indexChild, itemOut, indexOut) {
      ...
      if (itemChild.isChecked) {
        ...
        this.treeData.forEach((itemOutTemp, indexOutTemp) => {
          if (indexOutTemp != indexOut) {
            this.$set(itemOutTemp, "isChecked", false);
            this.treeData[indexOutTemp].children.forEach(
              (itemChildTemp, indexChildTemp) => {
    ...
    processDataNew(isReset) {
      //先整体初始化
      for (let index = 0; index < this.treeData.length; index++) {
        this.treeData[index].subjectClassifyName = this.treeData[
          index
        ].subjectClassifyName;
      ...
      if (isReset) {
        ...
        this.$set(
          this.treeData[this.queryParams.queryStatus - 1],
          "outIcon",
          "caret-down"
        );
        if (
          ...
          for (
            let index = 0;
            index < this.treeData[this.queryParams.queryStatus - 1].children.length;
            index++
          ) {
            for (
              let indexIn = 0;
              indexIn < this.queryParams.medicalId.length;
              indexIn++
            ) {
              if (
                this.queryParams.medicalId[indexIn] ==
                this.treeData[this.queryParams.queryStatus - 1].children[index]
                  .subjectClassifyId
              ) {
                this.$set(
                  this.treeData[this.queryParams.queryStatus - 1].children[index],
          ...
          for (let index = 0; index < this.queryParams.medicalId.length; index++) {
            for (
              ...
              if (
                this.treeData[this.queryParams.queryStatus - 1].children[indexIn]
                  .subjectClassifyId == this.queryParams.medicalId[index]
              ) {
